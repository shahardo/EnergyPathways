...
- **Row labels, axis names and channel names are data, not catalogue
  strings.** They come from the ingested workbook (`*_he`/`*_en` columns
  once T4/T5 land), not from `lib/i18n/locales/*/common.json`. The
  catalogue is UI chrome only (SPEC §8).
...
- Follow DEV-PLAN's dependency order (T1 → T2/T3 → T4 → T5 → T6 → T7 → T8–T13
  in parallel → T14 continuously). Don't build UI components against
  hand-mocked data if the ingestion pipeline (T5) is the actual blocker —
  build T5 first, or clearly stub with a documented placeholder like the
  current `components/workbook/README.md`.
